#!/usr/bin/env bash
# Builds RiaCore desktop application for macOS (DMG, arm64).
# Output: apps/desktop-host/dist-electron/
#
# Usage:
#   ./build-mac.sh              # full build (install + compile + package)
...
if [ -d "$OUT_DIR" ]; then
    find "$OUT_DIR" -maxdepth 1 -type f \( -name "*.dmg" -o -name "*.pkg" \) \
